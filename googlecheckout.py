...
class GoogleListener(webapp.RequestHandler):
    ...
    def manipulate_notification(self, notification_dict):
        ...
        if notification_processed: self.handshake(notification_dict['serial-number'])
        return
    ...
    def post(self): 
        if self.request.headers.get('Authorization'):
            ...
            if check_auth(merchant_id, merchant_key) and self.request.body:
                ...
                self.manipulate_notification(notification_dict)                  
            else: self.error(401)
        else: self.error(401)
        return
